...
func TestAuthMiddleware1ShouldAuth(t *testing.T) {
	...
	testTable := []struct {
		...
		{"OK", "admin", "admin", true, 200, "ok"},
		{"Wrong password", "admin", "admin2", false, 200, ""}, // The code is wrong, but the middleware works like this here
		{"No header", "", "", false, 200, ""},                 // Should test the missing auth header, I've added an if for this case in the code
	}
